...
public sealed class ImGuiRenderer
{
    ...
    public ImGuiRenderer(Game game)
    {
        ...
        // A window that throws halfway leaves ImGui's stacks unbalanced. By default that asserts and takes the
        // whole game down; with recovery on, ImGui closes the leftovers and carries on.
        var io = ImGui.GetIO();
        io.ConfigErrorRecovery = true;
        io.ConfigErrorRecoveryEnableAssert = false;
        // ImGui asserts if recovery is on but no reporting channel is enabled, so keep at least the debug log.
    ...
    public unsafe void RebuildFontAtlas()
    {
        ...
        unsafe { 
            ...
            {
                NativeMemory.Copy(pixelData, ptr, (nuint)pixels.Length); 
            }
        }
    ...
    /// <summary>
    /// Creates a pointer to a texture, which can be passed through ImGui calls such as Image. That pointer is then used by ImGui to let us know what texture to draw
    /// </summary>
    ...
    private void RenderDrawData(ImDrawDataPtr drawData)
    {
        ...
        var lastSampler = _graphicsDevice.SamplerStates[0];

        _graphicsDevice.BlendFactor = Color.White;
        _graphicsDevice.SamplerStates[0] = SamplerState.PointClamp; // sprite previews stay crisp
        ...
        _graphicsDevice.SamplerStates[0] = lastSampler;
    }
